...
public class SaOAuthClientController {
    ...
    @RequestMapping("/redictSuperset")
    public SaResult redictSuperset(@RequestBody SuperSetRequestData superSetRequestData) {
//         构造DashboardFilterClient的json字符串
        ObjectMapper mapper = new ObjectMapper();
        String filtersJson;
        try {
            filtersJson = mapper.writeValueAsString(superSetRequestData.getFilters());
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
        // 调用Server端接口，查询开放的资源
        String str = OkHttps.sync(serverUrl + "/oauth2/superset/videoSearch")
...
//		return "redirect:http://localhost:8088/superset/dashboard/1/";
    ...
    @Data
    private static class SuperSetRequestData {
        private String accessToken;
        private String slug;
        private List<DashboardFilterClient> filters;

    }


    @Data
    private static class DashboardFilterClient {
        private String name;
